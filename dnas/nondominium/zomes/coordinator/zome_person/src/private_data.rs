...
pub fn store_private_person_data(input: PrivatePersonDataInput) -> ExternResult<Record> {
    ...
    let record = get(private_data_hash.clone(), GetOptions::default())?
        .ok_or(PersonError::EntryOperationFailed("Failed to retrieve created private data".to_string()))?;
...
pub fn update_private_person_data(input: UpdatePrivatePersonDataInput) -> ExternResult<Record> {
    ...
    let record = get(updated_private_data_hash, GetOptions::default())?
        .ok_or(PersonError::EntryOperationFailed("Failed to retrieve updated private data".to_string()))?;
