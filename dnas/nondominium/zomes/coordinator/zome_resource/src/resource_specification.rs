use crate::{ResourceError, GovernanceRuleInput};
use hdk::prelude::*;
...
#[hdk_extern]
pub fn get_latest_resource_specification_record(original_action_hash: ActionHash) -> ExternResult<Option<Record>> {
    let links = get_links(
        GetLinksInputBuilder::try_new(original_action_hash.clone(), LinkTypes::ResourceSpecificationUpdates)?
            .build(),
    )?;
...
#[hdk_extern]
pub fn get_latest_resource_specification(original_action_hash: ActionHash) -> ExternResult<ResourceSpecification> {
    ...
    record
        ...
        .to_app_option()
        .map_err(|e| {
            ResourceError::SerializationError(format!("Failed to deserialize resource specification: {:?}", e))
        })?
        .ok_or(ResourceError::ResourceSpecNotFound("Resource specification entry not found".to_string()).into())
}
...
#[hdk_extern]
pub fn update_resource_specification(input: UpdateResourceSpecificationInput) -> ExternResult<Record> {
    ...
    let record = get(updated_spec_hash, GetOptions::default())?.ok_or(
        ResourceError::EntryOperationFailed("Failed to retrieve updated resource specification".to_string()),
    )?;
...
pub fn get_all_resource_specifications(_: ()) -> ExternResult<GetAllResourceSpecificationsOutput> {
    ...
    let links = get_links(
        GetLinksInputBuilder::try_new(path.path_entry_hash()?, LinkTypes::AllResourceSpecifications)?.build(),
    )?;
...
#[hdk_extern]
pub fn get_resource_specification_profile(action_hash: ActionHash) -> ExternResult<GetResourceSpecificationProfileOutput> {
    ...
    Ok(GetResourceSpecificationProfileOutput { specification: None })
}
...
pub fn get_resource_specification_with_rules(
    ...
    let spec = match spec_record.entry().to_app_option() {
        Ok(Some(s)) => s,
        _ => {
            return Err(ResourceError::SerializationError(
...
pub fn get_my_resource_specifications(_: ()) -> ExternResult<Vec<Link>> {
    let agent_info = agent_info()?;
    get_links(GetLinksInputBuilder::try_new(agent_info.agent_initial_pubkey, LinkTypes::AgentToOwnedSpecs)?.build())
}
...
pub fn get_resource_specifications_by_category(category: String) -> ExternResult<Vec<Record>> {
    ...
    let links = get_links(
        GetLinksInputBuilder::try_new(category_path.path_entry_hash()?, LinkTypes::SpecsByCategory)?.build(),
    )?;
