...
test("simple private data access test", async () => {
  await runScenarioWithTwoAgents(
    async (_scenario: Scenario, lynn: PlayerApp, bob: PlayerApp) => {
      ...
      // 1. Lynn creates person
      const lynnPerson = await createPerson(
        lynn.cells[0],
        samplePerson({ name: "Lynn" }),
      );
      console.log("✅ Lynn created person");

      // 2. Lynn stores private data
      const lynnPrivateData = await storePrivateData(
        lynn.cells[0],
        samplePrivateData({ email: "lynn@test.com" }),
      );
      console.log("✅ Lynn stored private data");
      ...
      console.log("✅ Lynn approved request", grantResponse);
...
test("private data sharing workflow for custodianship transfer", async () => {
  await runScenarioWithTwoAgents(
    async (_scenario: Scenario, lynn: PlayerApp, bob: PlayerApp) => {
      ...
      // Lynn (current custodian) creates her profile and private data
      const lynnPersonRecord = await createPerson(
        lynn.cells[0],
        samplePerson({
          name: "Lynn Custodian",
      ...
      assert.ok(lynnPersonRecord);

      const lynnPrivateDataRecord = await storePrivateData(
        lynn.cells[0],
        samplePrivateData({
          legal_name: "Lynn Marie Custodian",
          email: "lynn.custodian@example.com",
          phone: "+1-555-111-2222",
          location: "Seattle, WA",
          time_zone: "America/Los_Angeles",
        }),
      );
      assert.ok(lynnPrivateDataRecord);
      ...
      // 1. Bob requests access to Lynn's contact information for coordination
      ...
      // 2. Lynn checks her pending requests
      const pendingRequests = await getPendingDataRequests(lynn.cells[0]);
      ...
      console.log("Lynn sees pending request:", pendingRequests[0]);

      // 3. Lynn approves the request
      ...
      console.log(
        "Lynn approved the request and created grant:",
        grantResponse,
      );

      await dhtSync([lynn, bob], lynn.cells[0].cell_id[0]);

      // 4. Bob retrieves Lynn's shared contact information
      const sharedData = await getGrantedPrivateData(bob.cells[0], {
        target_agent: lynn.agentPubKey,
      ...
      assert.equal((sharedData as any).email, "lynn.custodian@example.com");
      ...
      console.log(
        "Bob successfully retrieved Lynn's contact info:",
        sharedData,
      );

      // 5. Verify Lynn can see her active grants
      const lynnGrants = await getMyDataGrants(lynn.cells[0]);
      assert.ok(Array.isArray(lynnGrants));
      assert.equal(lynnGrants.length, 1);
      assert.deepEqual(lynnGrants[0].granted_to, bob.agentPubKey);
      assert.equal(lynnGrants[0].context, "custodian_transfer_simulation");
      console.log("Lynn's active grants:", lynnGrants);

      // 6. Test grant revocation
      const grantHash = (grantResponse as any).grant_hash;
      assert.ok(grantHash, "Grant hash should exist");

      await revokeDataAccessGrant(lynn.cells[0], grantHash);
      console.log("Lynn revoked the data access grant");
...
test("direct data access grant without request", async () => {
  await runScenarioWithTwoAgents(
    async (_scenario: Scenario, lynn: PlayerApp, bob: PlayerApp) => {
      ...
      // Lynn directly grants access to Bob (note: this function doesn't exist in current impl)
      ...
      assert.equal((grantedData as any).email, "lynn@example.com");
...
test("data access validation and security", async () => {
  await runScenarioWithTwoAgents(
    async (_scenario: Scenario, lynn: PlayerApp, bob: PlayerApp) => {
      ...
      try {
        await requestPrivateDataAccess(lynn.cells[0], {
          requested_from: lynn.agentPubKey,
          fields_requested: ["email"],
          context: "self_test",
          resource_hash: null,
  ...
  await runScenarioWithTwoAgents(
    async (scenario: Scenario, lynn: PlayerApp, bob: PlayerApp) => {
      ...
      const lynnPrivateDataRecord = await storePrivateData(
        lynn.cells[0],
        samplePrivateData({
          legal_name: "Lynn Marie Governance",
          email: "lynn.governance@example.com",
          phone: "+1-555-GOVERN",
      ...
      const failedValidation = await validateAgentPrivateData(lynn.cells[0], {
        target_agent: lynn.agentPubKey,
...
test("debug private data retrieval issue", async () => {
  await runScenarioWithTwoAgents(
    async (scenario: Scenario, lynn: PlayerApp, bob: PlayerApp) => {
      ...
      // 1. Lynn needs a governance role to promote agents
      await createPerson(
        lynn.cells[0],
      ...
      console.log("✅ Lynn assigned governance role");

      // Sync after Lynn gets her role
      await dhtSync([lynn, bob], lynn.cells[0].cell_id[0]);
      ...
      try {
        ...
        console.log(
          "✅ Lynn verified Bob's self-validation:",
          verifiedValidation,
        );

        // Step 3: If verification succeeded, Lynn can proceed with promotion
  ...
  await runScenarioWithTwoAgents(
    async (scenario: Scenario, lynn: PlayerApp, bob: PlayerApp) => {
      ...
      await storePrivateData(
        lynn.cells[0],
        samplePrivateData({
          email: "lynn.auditor@example.com",
          phone: "+1-555-AUDIT",
          location: "Audit City",
        }),
      );
      ...
      const expiringGrants = await getExpiringGrants(lynn.cells[0], 8); // Look ahead 8 days (default grant is 7 days)
      ...
      const stats = await getPrivateDataSharingStats(lynn.cells[0]);
      assert.ok(stats);
      ...
      const bulkOperation = await executeBulkGrantOperation(lynn.cells[0], {
