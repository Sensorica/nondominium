...
impl ParticipationClaimType {
    ...
    pub fn description(&self) -> &'static str {
        match self {
            ...
            ParticipationClaimType::MaintenanceCommitmentAccepted => "Maintenance service commitment accepted",
            ParticipationClaimType::MaintenanceFulfillmentCompleted => "Maintenance service fulfillment completed",
            ParticipationClaimType::StorageCommitmentAccepted => "Storage service commitment accepted",
            ParticipationClaimType::StorageFulfillmentCompleted => "Storage service fulfillment completed",
            ParticipationClaimType::TransportCommitmentAccepted => "Transport service commitment accepted",
            ParticipationClaimType::TransportFulfillmentCompleted => "Transport service fulfillment completed",
            ParticipationClaimType::GoodFaithTransfer => "Good faith transfer to service provider",
            ParticipationClaimType::DisputeResolutionParticipation => "Constructive participation in dispute resolution",
            ParticipationClaimType::ValidationActivity => "Validation duties performed beyond specific transactions",
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (name, score) in scores {
            if !(0.0..=1.0).contains(&score) {
                return Err(format!("{} score must be between 0.0 and 1.0, got {}", name, score));
            }
        }
...
impl ReputationSummary {
    ...
    pub fn from_claims(
        ...
        let total_performance: f64 = claims.iter()
        ...
        for claim in &claims {
            match claim.claim_type {
                ...
                | ParticipationClaimType::ResourceValidation => creation_claims += 1,
                
                ParticipationClaimType::CustodyTransfer 
                | ParticipationClaimType::CustodyAcceptance => custody_claims += 1,
