...
pub fn create_governance_rule(input: GovernanceRuleInput) -> ExternResult<Record> {
    ...
    let record = get(rule_hash.clone(), GetOptions::default())?.ok_or(
        ResourceError::EntryOperationFailed(
            "Failed to retrieve created governance rule".to_string(),
        ),
    )?;
    ...
    record
        ...
        .map_err(|e| {
            ResourceError::SerializationError(format!(
                "Failed to deserialize governance rule: {:?}",
                e
            ))
        })?
        .ok_or(
            ResourceError::GovernanceRuleNotFound("Governance rule entry not found".to_string())
                .into(),
        )
}
...
pub fn update_governance_rule(input: UpdateGovernanceRuleInput) -> ExternResult<Record> {
    ...
    let record = get(updated_rule_hash, GetOptions::default())?.ok_or(
        ResourceError::EntryOperationFailed(
            "Failed to retrieve updated governance rule".to_string(),
        ),
    )?;
...
pub fn get_all_governance_rules(_: ()) -> ExternResult<GetAllGovernanceRulesOutput> {
    let path = Path::from("governance_rules");
    let links = get_links(
        GetLinksInputBuilder::try_new(path.path_entry_hash()?, LinkTypes::AllGovernanceRules)?
            .build(),
    )?;
...
pub fn get_governance_rules_by_type(rule_type: String) -> ExternResult<Vec<Record>> {
    ...
    let links = get_links(
        GetLinksInputBuilder::try_new(type_path.path_entry_hash()?, LinkTypes::RulesByType)?
            .build(),
    )?;
