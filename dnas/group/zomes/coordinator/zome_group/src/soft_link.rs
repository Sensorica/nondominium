...
pub fn create_soft_link(input: SoftLinkInput) -> ExternResult<Record> {
  ...
  // TODO(signals): remote_signal other members with { kind: SoftLink, group_hash }
  // so a newly-associated NDO appears in their group view push-style instead of
  // via poll/reload. See the consolidated design note in lib.rs.
