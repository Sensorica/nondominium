...
pub fn log_work(input: WorkLogInput) -> ExternResult<Record> {
  ...
  create_link(agent, work_log_hash, LinkTypes::AgentToWorkLogs, ())?;

  // TODO(signals): remote_signal other members with { kind: WorkLog, group_hash }
  // so a new work log appears in their group view push-style instead of via
  // poll/reload. See the consolidated design note in lib.rs.
