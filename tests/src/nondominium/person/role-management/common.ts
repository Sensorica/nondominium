...
export const getExpectedCapabilityLevel =
  personCommon.getExpectedCapabilityLevel;
...
export const ROLE_TEST_SCENARIOS: RoleTestScenario[] = [
  ...
  },
  {
    ...
    description:
      "Coordination level member with resource management capabilities",
...
export const ACCESS_CONTROL_SCENARIOS: AccessControlScenario[] = [
  ...
  },
  {
    ...
    roles: ["Resource Coordinator"],
    protected_actions: [
      "assign_resource_steward",
      "modify_resource_policies",
      "coordinate_resource_usage",
    ],
...
export async function testRoleProgressionScenario(
  ...
  for (let i = 0; i < pattern.role_sequence.length; i++) {
    ...
    const currentRoles: RoleType[] = currentRolesOutput.roles.map(
      (r) => r.role_name as RoleType,
    );
...
export function detectRoleConflicts(roles: RoleType[]): RoleConflict[] {
  ...
  const conflictMatrix: Record<
    string,
    {
      conflicts_with: RoleType[];
      reason: string;
      severity: "warning" | "error";
    }
  > = {
