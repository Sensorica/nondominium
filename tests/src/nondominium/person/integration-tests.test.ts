  ...
  async () => {
    await runScenarioWithTwoAgents(
      async (_scenario: Scenario, alice: PlayerApp, bob: PlayerApp) => {
        ...
        const aliceViewOfBob = await getPersonProfile(alice.cells[0], bob.agentPubKey);
  ...
  async () => {
    await runScenarioWithTwoAgents(
      async (_scenario: Scenario, alice: PlayerApp, bob: PlayerApp) => {
        ...
        const bobRolesFromLynn = await getPersonRoles(alice.cells[0], bob.agentPubKey);
        const bobRolesFromBob = await getPersonRoles(bob.cells[0], bob.agentPubKey);

        assert.equal(bobRolesFromLynn.roles.length, 1);
        assert.equal(bobRolesFromBob.roles.length, 1);
        assert.equal(bobRolesFromLynn.roles[0].role_name, TEST_ROLES.RESOURCE_STEWARD);
        assert.equal(bobRolesFromBob.roles[0].role_name, TEST_ROLES.RESOURCE_STEWARD);

        const aliceRolesFromLynn = await getPersonRoles(alice.cells[0], alice.agentPubKey);
        const aliceRolesFromBob = await getPersonRoles(bob.cells[0], alice.agentPubKey);

        assert.equal(aliceRolesFromLynn.roles.length, 1);
        assert.equal(aliceRolesFromBob.roles.length, 1);
        assert.equal(aliceRolesFromLynn.roles[0].role_name, TEST_ROLES.RESOURCE_COORDINATOR);
        assert.equal(aliceRolesFromBob.roles[0].role_name, TEST_ROLES.RESOURCE_COORDINATOR);

        // Verify capability checking from both agents
        const bobHasStewardFromLynn = await hasRoleCapability(
          alice.cells[0], bob.agentPubKey, TEST_ROLES.RESOURCE_STEWARD
        );
        const bobHasStewardFromBob = await hasRoleCapability(
          bob.cells[0], bob.agentPubKey, TEST_ROLES.RESOURCE_STEWARD
        );
        ...
        const aliceHasCoordinatorFromLynn = await hasRoleCapability(
          alice.cells[0], alice.agentPubKey, TEST_ROLES.RESOURCE_COORDINATOR
        );
        const aliceHasCoordinatorFromBob = await hasRoleCapability(
          bob.cells[0], alice.agentPubKey, TEST_ROLES.RESOURCE_COORDINATOR
        );
  ...
  async () => {
    await runScenarioWithTwoAgents(
      async (_scenario: Scenario, alice: PlayerApp, bob: PlayerApp) => {
        ...
        let aliceCapFromLynn = await getCapabilityLevel(alice.cells[0], alice.agentPubKey);
        let aliceCapFromBob = await getCapabilityLevel(bob.cells[0], alice.agentPubKey);
        let bobCapFromLynn = await getCapabilityLevel(alice.cells[0], bob.agentPubKey);
        let bobCapFromBob = await getCapabilityLevel(bob.cells[0], bob.agentPubKey);
        ...
        aliceCapFromLynn = await getCapabilityLevel(alice.cells[0], alice.agentPubKey);
        aliceCapFromBob = await getCapabilityLevel(bob.cells[0], alice.agentPubKey);
        bobCapFromLynn = await getCapabilityLevel(alice.cells[0], bob.agentPubKey);
  ...
  async () => {
    await runScenarioWithTwoAgents(
      async (_scenario: Scenario, alice: PlayerApp, bob: PlayerApp) => {
        ...
        const hasSteward = await hasRoleCapability(alice.cells[0], bob.agentPubKey, TEST_ROLES.RESOURCE_STEWARD);
        const hasCoordinator = await hasRoleCapability(alice.cells[0], bob.agentPubKey, TEST_ROLES.RESOURCE_COORDINATOR);
        ...
        const capabilityLevel = await getCapabilityLevel(alice.cells[0], bob.agentPubKey);
  ...
  async () => {
    await runScenarioWithTwoAgents(
      async (_scenario: Scenario, alice: PlayerApp, bob: PlayerApp) => {
        ...
        const bobProfileFromLynn = await getPersonProfile(alice.cells[0], bob.agentPubKey);
        ...
        const hasCapability = await hasRoleCapability(alice.cells[0], bob.agentPubKey, TEST_ROLES.RESOURCE_STEWARD);
        ...
        const bobProfileAfterCreation = await getPersonProfile(alice.cells[0], bob.agentPubKey);
        assert.ok(bobProfileAfterCreation.person);
        assert.equal(bobProfileAfterCreation.person!.name, "Bob");

        const bobRolesAfterCreation = await getPersonRoles(alice.cells[0], bob.agentPubKey);
