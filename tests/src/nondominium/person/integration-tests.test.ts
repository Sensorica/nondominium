  ...
  async () => {
    await runScenarioWithTwoAgents(
      async (_scenario: Scenario, alice: PlayerApp, bob: PlayerApp) => {
        ...
        const allAgentsFromLynn = await getAllAgents(alice.cells[0]);
        const allAgentsFromBob = await getAllAgents(bob.cells[0]);

        assert.equal(allAgentsFromLynn.agents.length, 2);
  ...
  async () => {
    await runScenarioWithTwoAgents(
      async (_scenario: Scenario, alice: PlayerApp, bob: PlayerApp) => {
        ...
        const bobRolesFromLynn = await getAgentRoles(alice.cells[0], bob.agentPubKey);
        ...
        assert.equal(bobRolesFromLynn.roles[0].role_name, TEST_ROLES.STEWARD);
        assert.equal(bobRolesFromBob.roles[0].role_name, TEST_ROLES.STEWARD);

        const aliceRolesFromLynn = await getAgentRoles(alice.cells[0], alice.agentPubKey);
        ...
        assert.equal(aliceRolesFromLynn.roles[0].role_name, TEST_ROLES.COORDINATOR);
  ...
  async () => {
    await runScenarioWithTwoAgents(
      async (_scenario: Scenario, alice: PlayerApp, bob: PlayerApp) => {
        ...
        let aliceCapFromLynn = await getAgentCapabilityLevel(alice.cells[0], alice.agentPubKey);
        let aliceCapFromBob = await getAgentCapabilityLevel(bob.cells[0], alice.agentPubKey);
        let bobCapFromLynn = await getAgentCapabilityLevel(alice.cells[0], bob.agentPubKey);
        let bobCapFromBob = await getAgentCapabilityLevel(bob.cells[0], bob.agentPubKey);

        assert.equal(aliceCapFromLynn, CAPABILITY_LEVELS.SIMPLE);
        assert.equal(aliceCapFromBob, CAPABILITY_LEVELS.SIMPLE);
        assert.equal(bobCapFromLynn, CAPABILITY_LEVELS.SIMPLE);
        assert.equal(bobCapFromBob, CAPABILITY_LEVELS.SIMPLE);

        // Lynn assigns accountable role to Bob
        await assignRole(alice.cells[0], sampleRole({
          role_name: TEST_ROLES.STEWARD,
        }, bob.agentPubKey));

        // Bob assigns primary role to Lynn
        await assignRole(bob.cells[0], sampleRole({
        ...
        aliceCapFromLynn = await getAgentCapabilityLevel(alice.cells[0], alice.agentPubKey);
        aliceCapFromBob = await getAgentCapabilityLevel(bob.cells[0], alice.agentPubKey);
        bobCapFromLynn = await getAgentCapabilityLevel(alice.cells[0], bob.agentPubKey);
        bobCapFromBob = await getAgentCapabilityLevel(bob.cells[0], bob.agentPubKey);

        assert.equal(aliceCapFromLynn, CAPABILITY_LEVELS.PRIMARY);
        assert.equal(aliceCapFromBob, CAPABILITY_LEVELS.PRIMARY);
        assert.equal(bobCapFromLynn, CAPABILITY_LEVELS.ACCOUNTABLE);
  ...
  async () => {
    await runScenarioWithTwoAgents(
      async (_scenario: Scenario, alice: PlayerApp, bob: PlayerApp) => {
        ...
        let allAgentsFromLynn = await getAllAgents(alice.cells[0]);
        // This might be 1 or 2 depending on DHT propagation timing
        assert.isAtLeast(allAgentsFromLynn.agents.length, 1);

        await dhtSync([alice, bob], alice.cells[0].cell_id[0]);

        // After DHT sync, both should see each other
        allAgentsFromLynn = await getAllAgents(alice.cells[0]);
        const allAgentsFromBob = await getAllAgents(bob.cells[0]);
  ...
  async () => {
    await runScenarioWithTwoAgents(
      async (_scenario: Scenario, alice: PlayerApp, bob: PlayerApp) => {
        ...
        const bobProfileFromLynn = await getAgentProfile(alice.cells[0], bob.agentPubKey);
