    ...
    T: std::fmt::Debug + for<'de> Deserialize<'de>,
{
    ...
    match zome_call_response {
        ZomeCallResponse::Ok(response) => {
            response.decode().map_err(|e| {
                CommonError::Serialize(format!("Failed to decode response: {e:?}")).into()
            })
        }
        _ => Err(CommonError::External(format!(
...
pub mod validation {
    ...
    pub fn validate_string_field(value: &str, field_name: &str, max_length: usize) -> Result<(), String> {
